...
exports.handler = async function(event) {
  ...
  try {
    ...
    if (stripeEvent.type === 'checkout.session.completed') {
      ...
      // Get customer email from Stripe customer object (customer_email is null when using customer ID)
      var customerEmail = session.customer_email;
      if (!customerEmail && session.customer) {
        try {
          var customer = await stripe.customers.retrieve(session.customer);
